...
namespace Commander
{
    ...
    public class Cog
    {
        ...
        public Cog(Program prog, string name = "", string description = "")
        {
            ...
            if (IsGroup) @group = Commands[Name];
            ...
            foreach (var method in methodInfo)
            {
                ...
                foreach (var a in attribs) examples.Add((a as Example).Val);
                ...
                if (group != null && parent == null)
                    @group.AddChild(cmd);
                else if (parent == null) Commands.Add(cmdName, cmd);
            }
        }
        ...
        public CommandObj GetCommand(string name)
        {
            CommandObj FindCommand(CommandObj cmd, string cmdName)
            {
                ...
                {
                    var co = FindCommand(child, cmdName);
        ...
        internal string Execute(Program prog, string name, string[] args)
        {
            ...
            {
                var next = "";
                if (cArgs.Length > 0) next = cArgs[0];

                // check if we can find the current command
                var cmd = GetCommand(iName);
                if (cmd == null) throw new CogError($"could not find command: {iName}");

                // check if next is subcommand
                var nextCmd = GetCommand(next);
                if (nextCmd == null) return cmd.Invoke(prog, cArgs);
